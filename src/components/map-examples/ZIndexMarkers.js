...
class ZIndexMarkers extends React.Component {
  ...
  render() {
    ...
    const markers = markerInfo.map(markInfo => (
      <Marker
        coordinate={markInfo}
        key={markInfo.id}
        pinColor={markInfo.isSpecial ? '#c5a620' : null}
        style={markInfo.isSpecial ? styles.specialMarker : null}
      />
    ));
